...
def main(game_name, game_length):
	...
	#Agent
	num_processes = 16
	experiment = "Experiments2" #Pretraining set to 0, 20k steps, remove debug, remove wrapper and trainer debug
	agent = PPOAgent(env, num_processes, experiment, lr=.00025, reconstruct=gen) #.00025

	agent.writer.add_hparams({'Experiment': experiment, 'lr':.00025, 'Minibatch':32, 'RL_Steps': 1e5, 'Notes':'Z Norm Target'}, {})
